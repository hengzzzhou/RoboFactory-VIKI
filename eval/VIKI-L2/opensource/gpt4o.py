...
def process_sample(idx, sample, output_dir, model):
    ...
    # Debug: Check if there are still any numpy arrays
    def check_for_arrays(obj, path=""):
        if isinstance(obj, np.ndarray):
            print(f"Warning: Found numpy array at {path}: {type(obj)}")
        elif isinstance(obj, dict):
            for key, value in obj.items():
                check_for_arrays(value, f"{path}.{key}")
        elif isinstance(obj, (list, tuple)):
            for i, item in enumerate(obj):
                check_for_arrays(item, f"{path}[{i}]")
    
    # Uncomment the next line for debugging
    # check_for_arrays(ground_truth, "ground_truth")
...
def main():
    ...
    datasets = {
        'test': {
            ...
            'output_dir': "eval/VIKI-L2/opensource/results/test"
        },
        'val': {
            'data_path': "RoboFactory-VIKI/data/viki/VIKI-L2/val.parquet",
            'output_dir': "eval/VIKI-L2/opensource/results/val"
        }
    }
