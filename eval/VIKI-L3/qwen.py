...
def process_sample(idx, sample, output_dir):
    ...
    # Add retry mechanism for zero scores
    max_retries = 3
    retry_count = 0
    ...
    while retry_count < max_retries:
        ...
        print(f"Task {task_id}, attempt {retry_count+1}: res={res}")
        
        if res > 0 or retry_count == max_retries - 1:
            break
            
        print(f"Got score 0, retrying ({retry_count+1}/{max_retries})...")
        retry_count += 1
...
def main():
    ...
    datasets = {
        'test': {
            ...
            'output_dir': "VIKI-L3/qwen/results/test"
        }
    }
    ...
    for dataset_name, dataset_info in datasets.items():
        ...
        data = data[:200]  # Limit to 200 samples for testing
